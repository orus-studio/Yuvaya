...
export default async function BlogDetailPage({ params }: PageProps) {
  ...
  const [article, allArticles] = await Promise.all([getArticleByHandle(id), getBlogPosts()]);

  if (!article) {
    return (
      <main className="w-full min-h-screen bg-[#fffff7] pt-32 pb-24 px-4 flex flex-col items-center justify-center text-center">
        <h1 className="font-cormorant text-5xl md:text-6xl text-[#26312d] mb-4">Story Not Found</h1>
        <p className="font-switzer text-gray-600 max-w-md mb-8">
          The story you are looking for doesn&apos;t exist or has been removed. Check back later or
